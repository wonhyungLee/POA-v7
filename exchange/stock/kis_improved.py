...
class ImprovedKoreaInvestment:
    ...
    def get_exchange_rate(self) -> Optional[float]:
        """현재 USD/KRW 환율 조회"""
        try:
            # 모의투자 여부에 따라 tr_id 동적 설정
            is_mock = "openapivts" in self.base_url
            tr_id = "VHDFS00000300" if is_mock else "HHDFS00000300"
            
            endpoint = "/uapi/overseas-stock/v1/quotations/inquire-price"
            headers = self._get_headers(tr_id) 
            params = {"AUTH": "", "EXCD": "NAS", "SYMB": "AAPL"} # 대표 종목으로 조회
            data = self._api_call_with_retry("GET", endpoint, headers, params)
            
            exchange_rate = float(data.get("output", {}).get("std_exrt", 0))
            if exchange_rate > 0:
                if self.debugger: self.debugger.logger.info(f"Current exchange rate: {exchange_rate} KRW/USD (tr_id: {tr_id})")
                return exchange_rate
            
            if self.debugger: self.debugger.logger.warning(f"Could not find exchange rate in API response. (tr_id: {tr_id})")
            return None
        except Exception as e:
            if self.debugger: self.debugger.logger.error(f"Failed to get exchange rate: {e}")
            return None
    ...
    def get_balance(self) -> Dict:
        ...
        try:
            ...
            if exchange_rate is None:
                print("WARNING: Live exchange rate unavailable, using fallback rate.")
                exchange_rate = 1350.0  # Fallback rate
